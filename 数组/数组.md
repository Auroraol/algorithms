...
+ ```c++
  // int n =  数组长度,
  for(int 1 = 0, r = 0; r < n; ++r)
  {
      //右边滑入窗口
      
      // while(不符合条件)  左边滑出窗口(letf++):  窗口数据不满足要求的  // 一般和区间大小有关
...
这个区间不等于滑动窗口, 这个区间是不固定的，由一个相同的值确定!
...
记忆:   用left的结果会遇到left == len的情况,    left + (right - left) / 2;     比他大左边就能肆无忌惮的移动
...
public class ListPrefixSum {

    private List<Integer> prefixSum;

    public ListPrefixSum(List<Integer> nums) {
        // 初始化前缀和列表
        prefixSum = new ArrayList<>();
        prefixSum.add(0); // 添加初始值0
    ...
    public static void main(String[] args) {
        ...
        ListPrefixSum listPrefixSum = new ListPrefixSum(nums);
        System.out.println(listPrefixSum.rangeSum(1, 3)); // 输出6，表示区间 [1, 3] 的累加和为6
    }
}
```
...
[STL equal()，equal_range()笔记 - 明明1109 - 博客园 (cnblogs.com)](https://www.cnblogs.com/fortunely/p/14566252.html)
...
+ 使用stl, 有srot()函数就直接用srot()函数。普通数组得手写
...
[Java 中使用 sort() 方法排序：从基本原理到多种用法全面总结](https://blog.csdn.net/weixin_72499901/article/details/136592073)

```java
//普通数组
Arrays.sort(array);   // 从小到大  // 自定义的要求排序 很麻烦
...
public class Main {
    public static void main(String[] args) {
        ...
        Arrays.sort(people); // 通过实现了Comparable接口, 进行排序
...
int main()
{
	int MAX;
	cin >> MAX;
	vector<char> isPrime(MAX + 10, 1);//先将所有元素都认为是素数
	vector<int> ans;
	for (int i = 2; i <= MAX; ++i)
	{
